...
# Output     : ann_model.pkl  (model + scaler saved for Streamlit)
# ============================================================
...
# ─────────────────────────────────────────────
print("=" * 65)
print("  ANN — HEART FAILURE PREDICTION")
print("=" * 65)
...
base_ann = MLPClassifier(random_state=42)
...
param_order = ["activation", "alpha", "early_stopping", "hidden_layer_sizes",
               "learning_rate", "learning_rate_init", "max_iter",
               "solver", "validation_fraction"]
...
if gap <= 3:
    ...
    print(f"    Verdict : Acceptable, consider increasing alpha (L2 regularisation)")
else:
    print(f"    Result  : SIGNIFICANT overfitting detected (gap = {gap:.2f}%)")
    print(f"    Verdict : Model memorised training data — increase alpha value")
...
fig.suptitle("ANN — Heart Failure Prediction Results",
...
bar_colors    = ["#185FA5", "#1D9E75", "#7F77DD", "#D85A30", "#BA7517"]
...
plt.show()
print(f"    Saved -> ann_results.png")

# ─────────────────────────────────────────────
# 11. SUMMARY TABLE
# ─────────────────────────────────────────────
print(f"\n{'=' * 65}")
print(f"  ANN MODEL — FINAL SUMMARY")
print(f"{'=' * 65}")
print(f"  Algorithm      : Artificial Neural Network (ANN / MLP)")
...
joblib.dump(model_data, "ann_model.pkl")

print(f"    Saved -> ann_model.pkl")
print(f"\n{'=' * 65}")
print(f"  Output files:")
print(f"    ann_model.pkl    — model + scaler for Streamlit")
print(f"    ann_results.png  — confusion matrix, ROC, metrics")
print(f"{'=' * 65}")